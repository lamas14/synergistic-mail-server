		...
		<div id="main_content">
			<div id="email_left_content">
				<div id="email_nav">
					<ul>
						...
						<li><a
							href="${pageContext.request.contextPath}/email/showEmails.htm?folderName=Inbox&page=0"
							title=""><b>Inbox</b></a></li>
							
						<li><a
							href="${pageContext.request.contextPath}/email/showEmails.htm?folderName=Send_Item&page=0"
							title=""><b>Send Item</b></a></li>
						<li><a class="current" href="#" title=""><b>Settings</b></a></li>

						<c:forEach var="item" items="${sessionScope.folderForms}">
							<li><a
								href="${pageContext.request.contextPath}/email/showEmails.htm?folderName=${item.folder}&page=0"
			...
			<div id="right_content">
				<table width="100%" align="center">
					...
					<c:forEach var="current" varStatus="count" items="${chatUserList}">
						<tr>
							<td>${count.index+1}</td>
							<c:choose>
								<c:when
									test="${current.fromuser eq sessionScope.userId}">
									<td>${current.touser}</td>
								</c:when>
								<c:otherwise>
									<td>${current.fromuser}</td>
								</c:otherwise>
							</c:choose>
							<td>${current.cdate}</td>
							<td><a href="${pageContext.request.contextPath}/chat/showHistory.htm?userid1=${current.touser}&userid2=${current.fromuser}">details</a> / <a href="${pageContext.request.contextPath}/chat/deleteHistory.htm">delete</a></td>
						</tr>
					</c:forEach>
				</table>
			</div>
